...
按 `todo.md`，迭代 1 到迭代 8 已完成，并且部分发布质量能力已经提前落地：
...
| 迭代 6 | 已完成 | 任务卡片、计划步骤状态、失败恢复入口、确定性详情、手动验收清单        |
| 迭代 7 | 已完成 | 计算、日期时间、排序、格式转换、文本统计等确定性工具增强与测试        |
| 迭代 8 | 已完成 | 记忆建议、冲突处理、搜索筛选、导入、命中解释和低风险偏好半自动记忆    |
...
当前阶段可以视为个人助手 MVP+：核心链路已经成型，并已补充全屏历史侧边栏、消息操作、引用清理、数据导入导出、内置使用说明、任务卡片、确定性工具增强、记忆系统升级和更严格的上下文策略。下一阶段重点是更多文档格式、网页资料增强和发布质量。
...
| 个人记忆       | `src/lib/assistantMemoryStore.ts`           | 结构化记忆 CRUD、搜索筛选、导入导出、建议、冲突检测、记住/忘记解析  |
...
| 确定性执行器   | `src/lib/assistantDeterministicExecutor.ts` | 计算、日期时间、排序、格式转换、文本统计                            |
...
| 确定性工具仍以轻量规则解析为主                          | 高复杂度公式、DST 时区、模糊自然语言仍需用户给出明确输入 |

## 下一阶段

下一阶段继续增强知识库导入类型、网页资料和发布质量：

1. 扩展知识库导入格式，例如 PDF、DOCX。
2. 增强网页剪藏：保存选区、正文、标题、URL、时间和标签。
3. 完善发布资料：README、隐私说明、排障文档和发布 checklist。
